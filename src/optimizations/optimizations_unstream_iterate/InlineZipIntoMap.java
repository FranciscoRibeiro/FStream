...
public class InlineZipIntoMap extends MasterBenchmarkUnstreamIterate{
    public static void main(String[] args) {
        System.out.println(MethodHandles.lookup().lookupClass().getSimpleName() + "...");

        InlineZipIntoMap izim = new InlineZipIntoMap();
                                    ...
                                    if (x11 instanceof Left) {
                                        Step aux2 = ((Function<Object, Step>) x2 -> {
                                            ...
                                            if (lAux.isEmpty()) {
                                                ...
                                                List<BigInteger> sub = lAux.subList(1, lAux.size());
                                                return new Yield<>(new Pair<>(((Triple) x1).getElem().get(), (BigInteger) lAux.get(0)), new Triple(((Triple) x1).getStateA(), new Left(sub), Optional.empty()));
                                            }
                                        ...
                                        Step aux2 = ((Function<Object, Step>) x2 -> {
                                            ...
                                            if (lAux.isEmpty()) {
                                                ...
                                                List<BigInteger> sub = lAux.subList(1, lAux.size());
                                                return new Yield<>(new Pair<>(((Triple) x1).getElem().get(), (BigInteger) lAux.get(0)), new Triple(((Triple) x1).getStateA(), new Right(sub), Optional.empty()));
                                            }
